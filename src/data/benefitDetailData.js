...
const PX_CALCULATOR = [
  {
    id: 'ibk', name: 'IBK 나라사랑', color: '#003DA5',
    summary: '기본 15~30% + 급여이체 시 최대 50%',
    conditions: '현역병 인정 + 급여이체 시 실적 면제',
    tiers: [
      { min: 0, max: 29999, rate: 0.15, label: '소액 기본할인 15%',
        dailyLimit: null, monthlyLimit: 10000, monthlyCount: 3,
        limitDetail: '월 3회, 월 한도 1만원',
        condition: '무실적 (현역병 인정 시)', type: 'basic' },
      { min: 30000, max: 99999, rate: 0.30, label: '3만원↑ 기본할인 30%',
        dailyLimit: null, monthlyLimit: 50000, monthlyCount: null,
        limitDetail: '월 한도 5만원 (횟수 무제한)',
        condition: '무실적 (현역병 인정 시)', type: 'basic' },
      { min: 30000, max: 99999, rate: 0.50, label: '3만원↑ 기본+특별 50%',
        dailyLimit: null, monthlyLimit: 50000, monthlyCount: null,
        limitDetail: '월 한도 5만원 (기본30%+특별20% 합산)',
        condition: '급여이체 + 전월실적 25만원', type: 'premium', bonus: true },
      { min: 100000, max: 999999, rate: 0.30, label: '10만원↑ 기본할인 30%',
        dailyLimit: null, monthlyLimit: 50000, monthlyCount: null,
        limitDetail: '월 한도 5만원',
        condition: '무실적 (현역병 인정 시)', type: 'basic' },
      { min: 100000, max: 999999, rate: 0.50, label: '10만원↑ 기본+특별 50%',
        dailyLimit: null, monthlyLimit: 50000, monthlyCount: null,
        limitDetail: '월 한도 5만원 (기본30%+특별20% 합산)',
        condition: '급여이체 + 전월실적 25만원', type: 'premium', bonus: true },
    ],
    notes: [
      '⚠️ 기본할인과 특별할인의 월 한도는 합산 5만원',
      '📋 현역병 인정: 장병내일준비적금 보유 시 자동 / 미보유 시 복무확인서 제출',
      '💰 급여이체: IBK 결제계좌로 병 급여 수령 실적 필요',
      '🔄 급여이체 충족 시 나라·사랑 영역 전월실적 면제',
      '📊 통합할인한도 없음 (3기 카드 중 IBK만의 강점)',
    ],
  },
  {
    id: 'hana', name: '하나 나라사랑', color: '#009775',
    summary: '소액 30% 무실적 / 고액 20%',
    conditions: '무실적 혜택 3사 최다, 별도 조건 없음',
    tiers: [
      { min: 0, max: 29999, rate: 0.30, label: '소액 30% 캐시백',
        dailyLimit: null, monthlyLimit: 5000, monthlyCount: null,
        limitDetail: '월 한도 5천원',
        condition: '무실적', type: 'basic' },
      { min: 30000, max: 99999, rate: 0.10, label: '3~10만원 10% 캐시백',
        dailyLimit: 5000, monthlyLimit: 20000, monthlyCount: null,
        limitDetail: '일 한도 5천원, 월 한도 2만원',
        condition: '무실적', type: 'basic' },
      { min: 100000, max: 999999, rate: 0.20, label: '10만원↑ 20% 캐시백',
        dailyLimit: 20000, monthlyLimit: 100000, monthlyCount: null,
        limitDetail: '일 한도 2만원, 월 한도 10만원',
        condition: '무실적', type: 'basic' },
    ],
    notes: [
      '✅ 모든 PX 혜택이 무실적 — 조건 맞추기 어려울 때 최적',
      '📋 소액(3만원 미만) 30%는 간식·음료 구매에 유리',
      '⚠️ 3~10만원 구간은 10%로 낮아지므로 주의',
      '💡 10만원 이상 대량 구매 시 20% (일 한도 2만원)',
    ],
  },
  {
    id: 'shinhan', name: '신한 나라사랑', color: '#0046FF',
    summary: '3만원↑ 20% 캐시백, 월 10만원 한도',
    conditions: 'VISA 브랜드, 전월실적 10만원/20만원 2단계',
    tiers: [
      { min: 0, max: 29999, rate: 0, label: '소액 1일1회 최대 1천원',
        dailyLimit: 1000, monthlyLimit: null, monthlyCount: 1,
        limitDetail: '1일 1회, 건당 최대 1천원 (월 한도 통합)',
        condition: '무실적', type: 'basic', fixedAmount: 1000, dailyCount: 1 },
      { min: 30000, max: 999999, rate: 0.20, label: '3만원↑ 20% 캐시백',
        dailyLimit: null, monthlyLimit: 100000, monthlyCount: null,
        limitDetail: '월 한도 10만원 (소액 포함 통합)',
        condition: '무실적', type: 'basic' },
    ],
    notes: [
      '✅ PX 할인은 무실적 — 전월실적 불필요',
      '📋 소액은 1일 1회 최대 1천원으로 혜택 작음',
      '💡 IBK 한도(5만원) 소진 후 신한(10만원)으로 전환하면 최적',
      '📊 월 10만원 한도로 3사 중 월 한도 가장 큼',
    ],
  },
  {
    id: 'kb', name: 'KB국민 (레거시)', color: '#FFBC00', isLegacy: true,
    summary: '20% 환급할인, 월 5만원 한도',
    conditions: '2025.12.24 신규발급 단종, 기발급자 최대 2030년 사용',
    tiers: [
      { min: 0, max: 999999, rate: 0.20, label: '20% 환급할인',
        dailyLimit: null, monthlyLimit: 50000, monthlyCount: null,
        limitDetail: '월 한도 5만원',
        condition: '없음', type: 'basic' },
    ],
    notes: [
      '⚠️ 신규 발급 불가 (2025.12.24 단종)',
      '📋 기발급자는 최대 2030년까지 사용 가능',
      '💡 다른 혜택(CGV 35%, 상해보험)도 유지됨',
    ],
  },
];
