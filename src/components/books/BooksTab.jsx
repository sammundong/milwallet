...
const BooksTab = ({ userData, updateUserData, handleBuy }) => {
  const [subCategory, setSubCategory] = useState('challenge');
  const [bookSubTab, setBookSubTab] = useState('selfDev');

  const subCategories = [
    { id: 'challenge', label: '독서챌린지' },
    { id: 'recommend', label: '추천도서' },
    { id: 'record', label: '독서기록' },
  ];
    ...
    <div>
      ...
      {subCategory === 'challenge' && (
        <>
          ...
          {readingRoutines.methods && (
            <div style={styles.section}>
              <h3 style={styles.sectionTitle}>📝 독서법 가이드</h3>
              {readingRoutines.methods.map((method, i) => (
                <div key={i} style={styles.card}>
                  <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>{method.emoji} {method.name}</div>
                  <div style={{ fontSize: 12, color: COLORS.textSecondary, lineHeight: 1.6 }}>{method.description}</div>
                </div>
              ))}
            </div>
          )}
