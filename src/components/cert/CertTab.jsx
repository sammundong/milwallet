...
const CertTab = ({ userData, handleBuy, setShowCertModal }) => {
  const [subCategory, setSubCategory] = useState('status');
  const [certSubTab, setCertSubTab] = useState('it');

  const subCategories = [
    { id: 'status', label: '나의자격증' },
    { id: 'routine', label: '합격루틴' },
        ...
        <div style={styles.section}>
          ...
          {certRoutines.map(routine => (
            <div key={routine.id} style={styles.card}>
              ...
              {/* Show related product names as text only, not full cards */}
                  ...
                  {routine.relatedProducts.map(pid => {
                    ...
                    return (
                      <div key={pid} style={{ fontSize: 12, color: COLORS.text, padding: '2px 0' }}>
                        • {p.name}
                        <button
                          style={{ marginLeft: 8, fontSize: 10, color: COLORS.primary, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
                          onClick={() => setSubCategory('textbooks')}
                        >교재 보기</button>
                      </div>
                    );
                  })}
